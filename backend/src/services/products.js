...
//     // ✅ Копіюємо точно ту ж логіку що і в getProducts
//     const workingPhotosFromDB = [
//       "https://i.ibb.co/X330FTj/shop-4-10-1000x1000-min.jpg",
//       "https://i.ibb.co/GxTVSVk/shop-4-9-1000x1000-min.jpg",
//       "https://i.ibb.co/02WmJdc/5-19-1000x1000-min.jpg",
...
//     // ✅ Обробляємо одинокий продукт так само як у getProducts
//     let photo = product.photo;

//     if (brokenUrls.includes(photo)) {
//       // ✅ Використовуємо хеш від productId для вибору зображення
...
// ✅ Функція для отримання консистентного індексу зображення за productId
const getImageIndexForProduct = (productId) => {
  const productIdStr = productId.toString();
  const hash = productIdStr
    .split("")
    .reduce((acc, char) => acc + char.charCodeAt(0), 0);
...
// ✅ Функція для виправлення поламаних зображень (використовуємо у обох функціях)
const fixProductImage = (product, fallbackIndex = 0) => {
  const workingPhotosFromDB = [
    "https://i.ibb.co/X330FTj/shop-4-10-1000x1000-min.jpg",
    "https://i.ibb.co/GxTVSVk/shop-4-9-1000x1000-min.jpg",
    "https://i.ibb.co/02WmJdc/5-19-1000x1000-min.jpg",
  ...
  if (brokenUrls.includes(photo)) {
    ...
    const imageIndex =
      getImageIndexForProduct(product._id) % workingPhotosFromDB.length;
    photo = workingPhotosFromDB[imageIndex];
  }
...
export const getProducts = async (filters = {}) => {
  ...
  try {
    ...
    // ✅ Використовуємо нову функцію для обробки зображень
    const processedProducts = products.map((product) =>
      fixProductImage(product)
    );
...
export const getProductById = async (productId) => {
  try {
    ...
    // ✅ Використовуємо ту саму функцію що і в getProducts
    const processedProduct = fixProductImage(product);
